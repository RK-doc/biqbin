...
//HEURISTIC FROM SAMUEL BURER (FROM C Code in GITHUB)
...
std::vector<int> Heuristic::getBestStableSetFound(int par_n, std::vector<Edge>& edges, int par_rank, int par_numReset, int maxSeconds)
{
	/*
	static int    Heuristic::n, Heuristic::m, Heuristic::oldest, Heuristic::iter, Heuristic::outeriter, Heuristic::tempint, Heuristic::maxcliquesize, Heuristic::rank;
	static int		Heuristic::beststablesize;
	static int*    Heuristic::edgesi, Heuristic::edgesj, Heuristic::stablesetsize, Heuristic::beststableset ;
	static int*    Heuristic::set1, Heuristic::set2, Heuristic::set3, Heuristic::set4;
	static int    Heuristic::size1, Heuristic::size2, Heuristic::size3, Heuristic::size4;
	static double Heuristic::tempval,  Heuristic::sigma, Heuristic::dirgradprod;
	static double* Heuristic::lambda, Heuristic::grad, Heuristic::projgrad, Heuristic::dir;
	static double  Heuristic::etabar, Heuristic::omegabar, Heuristic::tau, Heuristic::gammabar, Heuristic::alpha_omega;
	static double  Heuristic::beta_omega, Heuristic::alpha_eta, Heuristic::beta_eta, Heuristic::alpha_star, Heuristic::beta_star;
	static double  Heuristic::eta_star, Heuristic::omega_star, Heuristic::alpha_k, Heuristic::eta_k, Heuristic::omega_k;
	//FILE   *solnfile;
	static paramsH* Heuristic::par;
	static l_bfgs* Heuristic::vecs;
	*/


	//NEW:
	//-------
  std::vector<int> foundStableSet;

  int i, j, h, lambdacount, sigmacount, update, currmethod;
  int iteronstage, resetint;
  char edge, nonedge, **graph;
...
//  totaltime = 0.0;
  ...
  par = (paramsH*)malloc(sizeof(paramsH));
...
//  fclose(datafile);
  ...
  par->clique = 0;
  par->rank = par_rank;
  par->numresets = par_numReset;
  par->maxiter = 1000000;
  par->maxtime = maxSeconds;
  par->M = 2;
  par->savesoln = 1;
  //----------
...
//  }
  ...
  for(auto e: edges){
	  if(e.vertex1 > e.vertex2){
		  graph[e.vertex1][e.vertex2] = edge;
	  }
	  else{
		  graph[e.vertex2][e.vertex1] = edge;
	  }
  }
  //--------
...
//              fclose(solnfile);
            	...
            	//NEW:
            	//---------
            	foundStableSet.clear();
            	for(i = 1; i <= n; i++){
            		if(beststableset[i] == 1){
            			foundStableSet.push_back(i);
            		}
            	}
            	//----------
            }
...
/*      printf("increasing penalty (%d) ...\n\n", sigmacount); */
  ...
  return foundStableSet;

}



double Heuristic::reset(double* x, double* valu, double* valv, double* valedges)
{
  int i, j;
  double val, w1, w2;
...
double Heuristic::resetfull(double* x, double* valu, double* valv, double* valedges)
{
  int i, j;
  double val;
...
int Heuristic::checkmaximal(void)
{
  int i, j, k, *verify;
...
int Heuristic::stableset(double* x, double* valedges)
{
  int i, k, h;
  double t1, t2, eps;
  int *tempervec;
...
int Heuristic::normalize(double* x)
{
  int i;
  double norm;
...
double Heuristic::function(double* x, double* valu, double* valv, double* valedges)
{
  int i, j, h;
  double *u, *v, sum;
...
int Heuristic::gradient(double* x, double* valu, double* valv, double* valedges)
{
  int i, j, h;
...
int Heuristic::projgradient(double* x)
{
  int i;
...
int Heuristic::updateLBFGS(double a)
{
  int i;
...
double Heuristic::dot(double* vec1, double* vec2, int length)
{
  int i;
  double sum;
...
int Heuristic::addvec(double* vecnew, double* vec1, double scalar, double* vec2, int length)
{
  int i;
...
double Heuristic::infnorm(double* vec, int length)
{
  int i;
  double max;
...
double Heuristic::min(double a, double b)
{
  if(a < b) return a;
  else return b;
}

double Heuristic::max(double a, double b)
{
  if(a > b) return a;
  else return b;
}
...
double Heuristic::f_inexact_lsearch(double* x, double* valu, double* valv, double* valedges, double f_val, double* alpha, double alpha_upbd, double* ax, double* avalu, double* avalv, double* avaledges)
{
  /* define parameters */
...
double Heuristic::f_bracketing(double* x, double* valu, double* valv, double* valedges, double f_val, double sig, double rho, double tau1, double tau2, double tau3, double f_lower_bound, double* alpha, double alpha_upbd, double* ax, double* avalu, double* avalv, double* avaledges)
{
...
double Heuristic::f_sectioning(double* x, double* valu, double* valv, double* valedges, double* ax, double* avalu, double* avalv, double* avaledges, double rho, double tau2, double tau3, double sig, double a, double b, double f_zero, double df_zero, double f_a, double f_b, double df_a, double df_b, double *alpha, int polytype)
{
...
int Heuristic::f_interpolation_quadratic (double f_a, double df_a, double f_b, double a, double b, double a1, double b1, double *alpha)
{
  double za1, zb1, root;
  double endptmin;
...
clock_t Heuristic::GetTime(void)
{
  clock_t t ;
...
double Heuristic::TimeInSec(clock_t head,
                 clock_t rear)
{
//  double tscal;
//  tscal=0.01;
//	return ((double)(rear-head)*tscal);
  //NEW
  return ((double)(rear-head))/CLOCKS_PER_SEC;
}
