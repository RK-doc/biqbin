...
void outputFile(char *argv[], int n, int m, int sol[], double val, int counterBaBNodes, double timeTotal, double density, int optimal){
	...
	if (optimal)
		snprintf(fileOutput, sizeof(fileOutput), "output_files/%s%s", argv[2], "_solution_optimal.txt");
	else
		snprintf(fileOutput, sizeof(fileOutput), "output_files/%s%s", argv[2], "_solution_approximate.txt");
...
void outputFile_after_start(char *argv[], int n, int m, int sol[], double val,  int counterBaBNodes, double timeTotal, double density, int begin){
	...
	if (begin){
		...
		snprintf(fileOutput, sizeof(fileOutput), "output_files/%s%s", argv[2], "_start_info.txt");
		...
		snprintf(fileOutput, sizeof(fileOutput), "output_files/%s%s%d%s", argv[2], "_temporary_solution_", count, ".txt");
